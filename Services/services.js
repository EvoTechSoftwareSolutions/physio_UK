...
document.addEventListener("DOMContentLoaded", () => {
  const frontRadioButtons = document.querySelectorAll(
    'input[name="frontPain"]'
  );
  const backRadioButtons = document.querySelectorAll('input[name="backPain"]');
  const frontLabels = document.querySelectorAll(
    'input[name="frontPain"] + .radio-label'
  );
  const backLabels = document.querySelectorAll(
    'input[name="backPain"] + .radio-label'
  );
  ...
  frontLabels.forEach((label) => {
    label.addEventListener("mouseenter", (event) => {
      clearHover("front");
      const associatedRadio = document.getElementById(
...
function clearHover(section) {
  const labels = document.querySelectorAll(
    `input[name="${section}Pain"] + .radio-label`
  );
...
function changeImage(painType, section) {
  const image = document.getElementById(
    section === "front" ? "frontImage" : "backImage"
  );
  switch (painType) {
    case "neck":
    ...
    case "elbow":
      ...
      break;
    case "back":
      image.src = "../resources/img/services/service-back.png"; // Change to the correct path for the ankle image
      break;
    case "thigh":
      image.src = "../resources/img/services/service-thigh.png"; // Change to the correct path for the tricep image
      break;
    case "leg":
      image.src = "../resources/img/services/service-leg.png"; // Change to the correct path for the back image
      break;
    default:
